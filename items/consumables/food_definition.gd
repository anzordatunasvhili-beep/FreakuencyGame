...
enum BuffType { NONE, HEALTH_REGEN, ATTACK, DEFENSE, SPEED, LUCK }
...
@export_range(0, 100) var hunger_restore := 0

@export_group("Temporary Buff")
@export var buff_type := BuffType.NONE
@export var buff_value := 0.0
@export_range(0.0, 3600.0) var buff_duration := 0.0

@export_group("Use")
@export_range(0.0, 10.0) var consume_time := 0.6
...
func use(game_state: Node = null) -> bool:
	...
	return state != null and state.consume_food(self)
